...
export const MobileMenu = () => {
  ...
  let menuButtonLabel =
    layoutConfiguration.menuButtonLabel !== undefined
      ? layoutConfiguration.menuButtonLabel !== ""
        ? layoutConfiguration.menuButtonLabel
        : "menu"
      : "menu";
  let logoutButton = (data, key) => {
    return (
      <div key={"menu_button" + key}>
        ...
        >
          <Icon>{data.icon !== undefined ? data.icon : "logout"}</Icon>
          <ListItemText
            className={classes.menuItemText}
            primary={data.name !== undefined ? data.name : "Logout"}
          />
        </ListItem>
      </div>
    );
  ...
  let content = (
    <div className={classes.root}>
      ...
      >
        {headerRenderStatus() ? null : (
          <Divider className={classes.menuTitleDivider} />
        )}
        {items.map((value, i) => {
          if (!value.isFavourite) {
            if (value.type === "logout") return logoutButton(value, i);
            return (
